AFRAME.registerComponent('dashboard-item', {
  ...
  init() {
    ...
    const startSwayRotString = `${startRot.x} ${startRot.y-20} ${startRot.z}`
    const endSwayRotString = `${startRot.x} ${startRot.y + 20} ${startRot.z}`
    ...
    this.resetToHidden = () => {
      ...
      if (data.middlePos === true) {
        el.setAttribute('animation__comecloser', `property: position; to: 0 10 -12; easing: easeInOutSine; dur: 2000;`)
        setTimeout(() => {
          el.setAttribute('animation__comecloser', `property: position; to: ${hidePosString}; easing: easeInOutSine; dur: 1350;`)
        }, 2000);

      } else {
        el.setAttribute('animation__comecloser', `property: position; to: ${hidePosString}; easing: easeInOutSine; dur: 3350;`)
      }
      el.setAttribute('animation__scale', `property: scale; to: ${hideScaleString}; easing: easeInOutSine; dur: 3350;`)
      el.setAttribute('animation', `property: rotation; to: ${hideRotString}; easing: easeInOutSine; dur: 3350;`)
      el.addEventListener('animationcomplete', this.resetAnimations)
